...
library("pheatmap")
require("gplots")
files<-list.files("/scratch/AG_Ohler/jmuino/Julia/Tex-experiment/bamFiles/counts",pattern="counts$")
ids<-unlist(lapply(strsplit(files,".counts$"),function(x)x[1]))

res<-read.table(paste("/scratch/AG_Ohler/jmuino/Julia/Tex-experiment/bamFiles/counts/",files[1],sep=""),sep="\t",skip=2)$V7
gene<-read.table(paste("/scratch/AG_Ohler/jmuino/Julia/Tex-experiment/bamFiles/counts/",files[1],sep=""),sep="\t",skip=2)$V1
for (i in files[-1]){
temp<-read.table(paste("/scratch/AG_Ohler/jmuino/Julia/Tex-experiment/bamFiles/counts/",i,sep=""),sep="\t",skip=2)
...
dds <- DESeqDataSetFromMatrix(countData = resn,colData = colData,design = ~ cond+rep+geno+dev)
dds1 <- DESeq(dds)
vsd <- varianceStabilizingTransformation(dds1)
write.csv(as.data.frame(assay(vsd)),file="ALLGenesVSDNorm.csv")

 res<-read.csv("ALLGenesVSDNorm.csv");rownames(res)<-res$X;res$X<-NULL
pheatmap(as.matrix(res), cluster_rows=T, show_rownames=F,cluster_cols=T,scale="none",main="scale by row")#, annotation_row =colData)	


######
for(j in unique(dev)){
for(i in unique(geno)){
if(sum(geno==i & dev==j)>0){
print(i);print(j)
dds <- DESeqDataSetFromMatrix(countData = resn[,geno==i & dev==j],colData = colData[geno==i & dev==j,],design = ~ cond)
dds1 <- DESeq(dds)
res1<-results(dds1,contrast=c("cond","sam","con"),altHypothesis="greater")
write.csv(res1,file=paste(i,j,"Deseq2.csv",sep="_"))
...
mm<-read.csv(paste(unique(geno)[1],unique(dev)[1],"_Deseq2.csv",sep="\t"))
